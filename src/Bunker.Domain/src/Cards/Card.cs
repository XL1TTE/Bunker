...
/// </summary>
public abstract record Card(Id PublicId)
{
...
public record ProfessionCard(Id PublicId, string Profession) : Card(PublicId);
public record HobbiesCard(Id PublicId, string Hobbies) : Card(PublicId);
public record AgeCard(Id PublicId, int Age) : Card(PublicId);
public record SexCard(Id PublicId, SexCard.SexEnum Sex) : Card(PublicId)
{
    public enum SexEnum: byte { Male = 0, Female = 1 }
};
public record FactCard(Id PublicId, string Fact) : Card(PublicId);
